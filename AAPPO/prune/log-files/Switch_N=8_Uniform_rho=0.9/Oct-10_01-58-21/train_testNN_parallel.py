...
def run_policy(network_info, policy, simulators, scaler, logger, gamma, policy_iter_num, episodes, time_steps):
    ...
    if policy.prune is False:
        ...
        for s in simulators:
            s.set_weights_manually.remote(policy)
            # with torch.no_grad():
            #     ray.get(s).policynetwork.fc1.weight = torch.nn.Parameter(policy.policynetwork.fc1.weight)
            #     ray.get(s).policynetwork.fc2.weight = policy.policynetwork.fc2.weight
    ...
    if policy.prune is True:
        temp_policy = Policy(ray.get(network_info).N, policy.kl_targ, 
                             hid1_mult = policy.hid1_mult, alt_formulation = policy.alt_formulation)
        temp_policy.set_weights_manually(policy)
        temp_policy_weights = temp_policy.get_weights()
        torch.save(temp_policy_weights, file_weights)
...
def compute_target(trajectories, policy, network, val_func, gamma, lam, scaler, iteration, burn_in, val_import = False):
    """
    compute value function for further training of Value Neural Network
    #modification for switch network: instead of compute expectation explicitly, use approximation
    ...
    ######### normalization ########################
    if val_import is False and iteration == 1:
...
def main(network_id, num_policy_iterations, gamma, lam, kl_targ, number_of_actors, hid1_mult, episode_length,
         burn_in, clipping_parameter, episode_length_evaluation, target_iter, 
         val_formulation = False, pol_formulation = True, 
         val_import = False, pol_import = False, prune_start = 0.5, prune_level = 0.1):
    """
    # Main training loop
    ...
    arrival = ray.get(network_id).arrival_type
    ...
    if pol_import:
        ...
        cwd = os.getcwd()
        scaler_path = cwd + '/N='+ str(N) +'_'+ arrival + '/scaler.npy'        
        policy_path = cwd + '/N='+ str(N)+'_' + arrival + '/weights.pth'
        ...
        initial_states_set = np.zeros((1,obs_dim))
        trajectories, total_steps, action_distr = \
            policy.run_episode(ray.get(network_id), scaler, episode_length * 10, initial_states_set[0], rpp=True)
    ...
    if val_import:
        ...
        cwd = os.getcwd()       
        val_path = cwd + '/N='+ str(N) +'_'+ arrival +'/val.pth'
        
        val_weights = torch.load(val_path)
        val_func.set_weights(val_weights)
    ...
    while iteration < num_policy_iterations:
        ...
        while ti_iter<target_iter:
            ...
            state_observes, port_observes, disc_sum_rew_norm = compute_target(trajectories, policy, ray.get(network_id), val_func, gamma, lam, scaler,iteration, burn_in, val_import = val_import)  # calculate values from data
        ...
        if iteration > int ( prune_start * num_policy_iterations) and policy.prune is False:
            policy.prune_policy(prune_level)
