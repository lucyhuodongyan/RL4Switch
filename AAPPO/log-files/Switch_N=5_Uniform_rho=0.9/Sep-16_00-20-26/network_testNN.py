...
class SwitchNetwork:
    ...
    def __init__(self, N, rho, arrival_type):
        ...
        if self.arrival_type == "uniform":
            ...
            power = 1
            norm_constant = np.sum(np.arange(self.N + 1) ** power)
            self.lambda_matrix = np.repeat([np.repeat([1/norm_constant], self.N)], self.N, axis = 0)
            for i in range(1, self.N):
                self.lambda_matrix[i:, i:] = (i + 1)**power / norm_constant
            self.lambda_matrix = self.lambda_matrix * self.rho
            # self.lambda_matrix[np.arange(self.N), np.arange(self.N)] = self.lambda_matrix[np.arange(self.N), np.arange(self.N)] + (self.rho - np.sum(self.lambda_matrix, axis = 1))
            self.lambda_matrix = self.lambda_matrix.reshape(-1)
        
        elif self.arrival_type == "skewed_block":
            ratio = 1/10
            block_width = max(1, int(self.N *0.75))
            block_1 = np.repeat(ratio / block_width, block_width**2).reshape((block_width, block_width))
            block_2 = np.repeat(ratio / block_width, block_width * (self.N - block_width)).reshape((block_width, self.N - block_width))
            block_3 = np.repeat((1 - ratio) / (self.N - block_width), (self.N - block_width)**2).reshape((self.N - block_width, self.N - block_width))
            self.lambda_matrix = np.block([[block_1, block_2], [block_2.transpose(), block_3]]).reshape(-1) * self.rho
