...
class Policy(object):
    ...
    def prune_policy(self, sparsity = 0.5):

        prune.l1_unstructured(self.policynetwork.fc1, name = "weight", amount = sparsity)
        prune.remove(self.policynetwork.fc1, "weight")
    ...
    def run_episode(self, network, scaler, time_steps, initial_state, rpp = False, stochastic = True):
        ...
        while t < time_steps: # run until visit to the empty state (regenerative state)
            ...
            while k < network.N :
                ...
                # array_actions[t, k] = act_distr_normalized
    ...
    def initialize_rpp(self, state_obs, port_availability, action_distr, batch_size=2048):
        ...
        x_train, y_train = torch.Tensor(pol_observes), torch.clamp(torch.Tensor(action_distr), 1e-20, 1)
        ...
        for e in range(20):
            ...
            for local_x, local_y in train_loader:
                ...
                pred_y_clip = torch.clamp(pred_y, 1e-20, 1)
                loss_val = self.kl_loss(torch.log(pred_y_clip), torch.Tensor(local_y)) # Compute Loss 
                # loss_val = torch.mean(torch.sum(local_y * torch.log(torch.div(local_y, pred_y_clip)), axis=1))
