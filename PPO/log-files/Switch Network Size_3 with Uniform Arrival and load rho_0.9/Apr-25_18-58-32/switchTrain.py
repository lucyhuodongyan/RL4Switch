...
def run_policy(network_id, policy, simulators, scaler, logger, gamma,
    ...
    for j in range(actors_per_run):
        accum_res.extend(ray.get([simulators[i].run_episode.remote(network_id, scaler_id, time_steps,
                                 initial_states_set) for i in range(MAX_ACTORS)]))
    if remainder>0:
        accum_res.extend(ray.get([simulators[i].run_episode.remote(network_id, scaler_id, time_steps,
                                 initial_states_set) for i in range(remainder)]))
    print('simulation is done')
...
def compute_val(trajectory, network, val_func, policy, scaler):
    ...
    scale, offset = scaler.get()
    val_input_mat_for_val_nn_norm = (val_input_mat_for_val_nn - offset[:-1]) * scale[:-1]
    state_val = val_func.predict(val_input_mat_for_val_nn_norm)
...
def compute_next_val_model(trajectory, network, val_func, policy, scaler):
    ...
    for act in range(network.action_size):
        ...
        next_state_norm = (next_state - offset[:-1]) * scale[:-1]
        ...
        pol_array = policy.sample(next_state_norm)
...
def add_value(trajectories, network, val_func, policy, scaler):
    ...
    for trajectory in trajectories:
        ...
        #Note the difference between switch network add_value and Mark's code, is that in Mark's case, next state can be explictly calculated,
        #

        # # approximate value function for the possible next states
        # values_set = np.zeros(( len(possible_states)+1, len(trajectory['observes'])))

        # new_obs = (trajectory['unscaled_obs'] - offset[:-1]) * scale[:-1]
        ...
        #     new_obs =(trajectory['unscaled_obs'] + trans - offset[:-1]) * scale[:-1]
        #     values = val_func.predict(new_obs)
        #     values = values / scale[-1] + offset[-1]
...
def main(network_id, num_policy_iterations, gamma, lam, kl_targ, number_of_actors, hid1_mult, episode_length,
         burn_in, clipping_parameter, episode_length_evaluation, target_iter, policy_model, q_model):
    """
    # Main training loop
    ...
    trajectories, total_steps, action_distr = \
        policy.run_episode(ray.get(network_id), scaler, episode_length * obs_dim, initial_states_set[0],rpp=True, mw1 = False)
    state_input = (trajectories['unscaled_obs'] - offset[:-1]) * scale[:-1]
    policy.initialize_rpp(state_input, action_distr)
    
    ma_cost = np.repeat(np.mean(np.sum(trajectories['unscaled_obs'],axis=1)) * 2,5) # initially *2, just to avoid cut the alpha rate in the first round
