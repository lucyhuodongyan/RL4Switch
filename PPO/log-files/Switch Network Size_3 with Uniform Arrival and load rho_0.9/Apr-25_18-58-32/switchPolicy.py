...
class Policy(object):
    ...
    def surr_loss(self, states, actions, old_prob, advantages):
        ...
        prob_ratio = torch.exp(torch.log(torch.clamp(new_prob, eps, 1.0)) - torch.log(torch.clamp(old_prob, eps, 1.0)))
    ...
    def update(self, observes, actions, advantages, logger):
        ...
        actions_probs = self.policynetwork(torch.Tensor(observes)).detach()
        ratios = torch.exp(torch.log(torch.clamp(actions_probs, eps, 1.0)) - torch.log(torch.clamp(actions_prob_old_train, eps, 1.0)))
        
        one_hot = torch.zeros(np.shape(actions)[0], self.act_dim)
        one_hot[torch.arange(np.shape(actions)[0]), actions] = 1
        ratios_act = torch.sum(torch.mul(torch.Tensor(ratios), one_hot), axis=1)
        ...
        print('policy training time: {:.3f}...'.format(int(((datetime.datetime.now() - policy_training_start).total_seconds() / 60) * 100) / 100.), 'minutes')
