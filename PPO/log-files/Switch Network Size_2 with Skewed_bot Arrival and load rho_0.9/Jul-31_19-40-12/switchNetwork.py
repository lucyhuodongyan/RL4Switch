...
class SwitchNetwork:
    ...
    def __init__(self, N, rho, arrival_type):
        ...
        if self.arrival_type == "uniform":
            ...
            self.lambda_matrix = np.repeat([np.repeat([1 / np.sum(np.arange(self.N + 1))], self.N)], self.N, axis = 0)
            for i in range(1, self.N):
                self.lambda_matrix[i:, i:] = (i + 1) / np.sum(np.arange(self.N + 1))
            self.lambda_matrix = self.lambda_matrix.reshape((1, self.N **2))[0] * self.rho
        else:
