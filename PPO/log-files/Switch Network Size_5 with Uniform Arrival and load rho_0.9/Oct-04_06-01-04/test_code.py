...
# p_model_vector = [False, True, True]
# q_model_vector = [False, False, True]
# above corresponds to SARSA, Expected SARSA, Exp(Everything)

p_model_vector = [False]
q_model_vector = [False]

for N in N_vector:
    for r in rho_vector:
        network = sn.SwitchNetwork(N,r,"uniform") # queuing network declaration
        ...
        for ti in ti_vector:
            ...
            for i in range(len(p_model_vector)):
                args.policy_model = p_model_vector[i]
                args.q_model = q_model_vector[i]
