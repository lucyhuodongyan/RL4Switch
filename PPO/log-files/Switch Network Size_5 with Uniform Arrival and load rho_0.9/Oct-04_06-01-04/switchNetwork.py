...
class SwitchNetwork:
    ...
    def mw_alpha(self, state):
        Q_policy=np.zeros(self.action_size)
        for i in range(self.action_size):
            policy= self.dict_absolute_to_matrix_action[i]
            Q_policy[i]=np.sum(np.multiply(np.power(state, 1),policy))

        maxQ = max(Q_policy)
        pol_candidate = np.where(Q_policy ==maxQ)[0]
        
        pol_distr=np.zeros(self.action_size)
        pol_distr[pol_candidate] = 1/len(pol_candidate)
